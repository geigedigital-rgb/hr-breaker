  ...
  return (
    <div className="flex min-w-0 items-center gap-3">
      {item.company_logo_url ? (
        ...
        <div
          className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#F5F6FA] text-xs font-semibold text-[var(--accent)]"
          aria-hidden
        >
          {initial}
        </div>
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 text-sm text-[var(--text-muted)]" role="status" aria-live="polite">
        <span className="h-4 w-4 animate-spin rounded-full border-2 border-[var(--accent)] border-t-transparent" aria-hidden />
        {t("history.loading")}
      </div>
    );
  }
    ...
    <div className="space-y-8">
        ...
        <section aria-labelledby="latest-heading" className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {latest.map((item) => (
              <article
                key={item.filename}
                className="flex flex-col rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 transition-colors hover:border-[var(--accent)]/40 hover:bg-[var(--accent-soft)]/80"
              >
                ...
                <div className="mt-4 flex gap-2">
                  {hasPaidPlan ? (
                    <a
                      ...
                      className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-[var(--border)] bg-[#F5F6FA] px-3 py-2 text-sm font-medium text-[var(--text)] transition-colors hover:border-[var(--accent)] hover:bg-[var(--accent)] hover:text-white"
                    >
                    ...
                    <span
                      className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-[var(--border)] bg-[#F5F6FA] px-3 py-2 text-sm font-medium text-[var(--text-muted)] opacity-60 cursor-not-allowed"
                  ...
                  {hasPaidPlan ? (
                    <a
                      ...
                      className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-[var(--border)] bg-[#F5F6FA] px-3 py-2 text-sm font-medium text-[var(--text)] transition-colors hover:border-[var(--accent)] hover:bg-[var(--accent)]/10 hover:text-[var(--accent)]"
                    >
                    ...
                    <span
                      className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-[var(--border)] bg-[#F5F6FA] px-3 py-2 text-sm font-medium text-[var(--text-muted)] opacity-60 cursor-not-allowed"
      ...
      <section aria-labelledby="all-history-heading" className="space-y-4">
        ...
        <div className="ds-card overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-end sm:justify-between gap-4 border-b border-[var(--border)] px-4 sm:px-5 py-4 bg-[#F5F6FA]/50">
            <div className="min-w-0 flex-1 w-full sm:max-w-xs">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="h-10 w-full ds-card !rounded-[var(--radius-md)] py-2 pl-9 pr-3 text-sm text-[var(--text)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/30 focus:border-[var(--accent)]"
            ...
            <Listbox value={period} onChange={setPeriod}>
              <div className="flex flex-col w-full sm:w-auto">
                ...
                <div className="relative w-full sm:w-auto">
                  <ListboxButton
                    aria-label="Период"
                    className="flex items-center gap-2 h-10 w-full sm:min-w-[11rem] ds-card !rounded-[var(--radius-md)] px-3 pr-9 text-left text-sm text-[var(--text)] transition-colors hover:border-[#E0E2E8] hover:bg-[#FAFBFC] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/30 focus:border-[var(--accent)]/50"
                  >
                  ...
                  <ListboxOptions
                    anchor="bottom end"
                    className="z-20 mt-1.5 min-w-[11rem] ds-card !rounded-[var(--radius-md)] py-1 shadow-[0_4px_12px_rgba(0,0,0,0.06)] focus:outline-none [--anchor-gap:4px]"
                  >
                    {PERIOD_OPTIONS.map((opt) => (
                      <ListboxOption
                        key={opt.value}
                        value={opt.value}
                        className="cursor-pointer py-2.5 px-3 text-sm text-[var(--text)] data-[focus]:bg-[#F5F6FA] data-[selected]:bg-[#EBEDF5] data-[selected]:font-medium data-[selected]:text-[var(--text)]"
                      >
                        {opt.label}
                      </ListboxOption>
                    ))}
                  ...
                  <li key={item.filename} className="px-4 py-3.5 space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-semibold text-[var(--text)] truncate" title={item.job_title || undefined}>
                          {item.job_title || "—"}
                        </p>
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-1">
                      {hasPaidPlan ? (
                        <a
                          href={api.downloadUrl(item.filename, api.getStoredToken())}
                          download={item.filename}
                          className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-[var(--border)] bg-[#F5F6FA] px-3 py-2 text-xs font-medium text-[var(--text)] transition-colors hover:border-[var(--accent)] hover:bg-[var(--accent)]/8"
                        >
                          <ArrowDownTrayIcon className="h-4 w-4" aria-hidden />
                          PDF
                        </a>
                        ...
                        <span
                          className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-[var(--border)] bg-[#F5F6FA] px-3 py-2 text-xs font-medium text-[var(--text-muted)] opacity-60 cursor-not-allowed"
                          title="PDF export requires a paid plan"
                        >
                          <ArrowDownTrayIcon className="h-4 w-4" aria-hidden />
                          PDF
          ...
          <div className="hidden lg:block">
            ...
            <div
              className="grid grid-cols-[2rem_minmax(0,1fr)_minmax(0,0.85fr)_minmax(0,1.6fr)_4.5rem_minmax(0,0.9fr)_6rem] gap-x-3 px-5 py-2.5 border-b border-[var(--border)] bg-[#F5F6FA]/70 text-xs font-medium text-[var(--text-muted)]"
              aria-hidden
            >
              <span />
                    ...
                    {({ open }) => (
                      <>
                        <DisclosureButton
                          ...
                          role="button"
                          tabIndex={0}
                          className="grid grid-cols-[2rem_minmax(0,1fr)_minmax(0,0.85fr)_minmax(0,1.6fr)_4.5rem_minmax(0,0.9fr)_6rem] gap-x-3 w-full items-center px-5 py-2.5 text-left text-sm transition-colors hover:bg-[var(--accent-soft)]/80 focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/30 focus:ring-inset rounded-none cursor-pointer"
                          aria-expanded={open}
                        >
                          <span
                          ...
                          <span className="min-w-0 truncate text-[var(--text)]" title={personName}>
                            {personName}
                          </span>
                          <span className="min-w-0 truncate text-[var(--text)]" title={item.job_title || undefined}>
                            {item.job_title || "—"}
                          </span>
                          ...
                          >
                            {hasPaidPlan ? (
                              <a
                                href={api.downloadUrl(item.filename, api.getStoredToken())}
                                download={item.filename}
                                className="inline-flex h-7 w-7 items-center justify-center rounded-md bg-transparent text-[var(--text-muted)] hover:bg-[#EBEDF5] hover:text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/30"
                            ...
                            <button
                              ...
                              disabled={deleting === item.filename}
                              className="inline-flex h-7 w-7 items-center justify-center rounded-md bg-transparent text-[var(--text-muted)] hover:bg-[#EBEDF5] hover:text-red-600 focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/30 disabled:opacity-50"
                        ...
                        <DisclosurePanel className="border-t border-[var(--border)] bg-[#FAFBFC] px-5 py-3 pl-[3.25rem]">
                          <div className="flex flex-wrap items-center gap-2 text-sm">
                            {(item.first_name || item.last_name) && (
                              <span className="rounded-lg bg-[#EBEDF5] px-2.5 py-1.5 text-[var(--text)]">
                            ...
                            {item.job_url && (
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--border)] bg-white px-2.5 py-1.5 text-[var(--text)] transition-colors hover:bg-[var(--accent-soft)] hover:border-[#E0E2E8] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/30"
                              >
