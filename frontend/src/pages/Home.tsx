...
export default function Home() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Link
        to="/improve"
        className="block rounded-2xl overflow-hidden focus:outline-none focus:ring-2 focus:ring-white/40 focus:ring-offset-2 focus:ring-offset-[var(--accent)]"
        style={{ background: "var(--grad-accent-btn)" }}
      >
        ...
        <div className="relative px-4 py-4 md:hidden">
          ...
          <div className="mt-4">
            <span className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-white px-4 py-2.5 text-sm font-semibold text-[var(--accent)] shadow-sm transition-opacity hover:opacity-90">
              {t("home.startNow")}
        ...
        <div className="relative hidden min-h-[11rem] items-center gap-4 overflow-hidden px-6 py-5 md:flex">
          ...
          <div className="relative mr-6 shrink-0">
            <span className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2.5 text-sm font-semibold text-[var(--accent)] shadow-sm transition-opacity hover:opacity-90">
              {t("home.startNow")}
              <svg className="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
              </svg>
      ...
      <section className="ds-card overflow-hidden">
        <div className="px-5 py-4 border-b border-[var(--border)]">
          <h2 className="text-base font-semibold text-[var(--text)]">{t("home.myResumes")}</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {documents.slice(0, 10).map((item) => {
                ...
                return (
                  <article
                    key={item.filename}
                    className="flex gap-3 rounded-xl p-2.5 hover:bg-[var(--accent-soft)] transition-colors min-w-0"
                  >
                    ...
                    <div className="shrink-0 w-44 min-w-0 rounded-md bg-white overflow-hidden flex flex-col relative aspect-[210/297] group/preview">
                      <a
                        ...
                        title={t("home.open")}
                      >
                        <span className="opacity-0 group-hover/preview:opacity-100 transition-opacity flex items-center justify-center w-8 h-8 rounded-full bg-white/95 text-[var(--text)] shadow-sm">
                      ...
                      {thumbnailErrors.has(previewFilename) ? (
                        <>
                          ...
                          <div className="flex-1 p-1.5 flex flex-col justify-center">
                            <DocumentTextIcon className="w-6 h-6 text-[var(--accent)]/60 mx-auto mb-1" />
                            <p className="text-[9px] font-semibold text-[var(--text)] uppercase tracking-wide text-center truncate">
                              {name}
                            </p>
                          </div>
                        </>
                    ...
                    <div className="min-w-0 flex-1 flex flex-col py-0.5">
                      <h3 className="text-[13px] font-semibold text-[var(--text)] truncate leading-tight">{name}</h3>
                      ...
                      <div className="flex flex-col gap-0.5 mt-1.5">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 p-1.5 rounded-lg text-xs font-medium text-[var(--text-muted)] hover:bg-white hover:text-[var(--accent)] hover:shadow-[var(--shadow-sm)] transition-colors disabled:opacity-50 text-left"
                        >
                          {improvingFilename === item.filename ? (
                            <span className="w-3.5 h-3.5 border border-[var(--accent)]/30 border-t-[var(--accent)] rounded-full animate-spin shrink-0" aria-hidden />
                          ) : (
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 p-1.5 rounded-lg text-xs font-medium text-[var(--text-muted)] hover:bg-[var(--danger-soft)] hover:text-[var(--danger)] transition-colors text-left"
                        >
              ...
              <button
                ...
                className="flex gap-3 rounded-xl p-2.5 hover:bg-[var(--accent-soft)] transition-colors text-left disabled:opacity-60 disabled:cursor-wait group/add"
              >
                {/* A4-shaped dashed area — same proportions as resume preview */}
                <div className="shrink-0 w-44 aspect-[210/297] rounded-md border-2 border-dashed border-[#c8cddc] group-hover/add:border-[var(--accent)]/50 transition-colors flex items-center justify-center">
                  {addUploading ? (
                    <span className="h-7 w-7 border-2 border-[var(--accent)]/30 border-t-[#4578FC] rounded-full animate-spin" aria-hidden />
                  ) : (
                    <span className="w-12 h-12 rounded-full bg-[var(--accent-soft)] group-hover/add:bg-white flex items-center justify-center transition-colors shadow-[var(--shadow-sm)]">
                      <PlusIcon className="w-6 h-6 text-[var(--text-tertiary)] group-hover/add:text-[var(--accent)] transition-colors" strokeWidth={2} />
                    </span>
                  )}
                </div>
                {/* Text block */}
      ...
      {deleteConfirm && (
        ...
        >
          <div
            className="ds-card shadow-xl max-w-sm w-full p-5"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 id="delete-dialog-title" className="text-base font-semibold text-[var(--text)]">
            ...
            <div className="mt-5 flex gap-3 justify-end">
              <button
                ...
                onClick={() => !deleting && setDeleteConfirm(null)}
                disabled={deleting}
                className="px-4 py-2 rounded-xl border border-[var(--border)] bg-white text-sm font-medium text-[var(--text)] hover:bg-[var(--accent-soft)] hover:text-[var(--accent)] transition-colors disabled:opacity-60"
              >
                {t("home.cancel")}
              </button>
              <button
