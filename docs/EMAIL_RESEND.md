...
# Необязательно: «живой» Reply-To (например support@домен) — письмо выглядит ближе к обычной переписке, не только как рассылка.
RESEND_REPLY_TO=
```

Если `EMAIL_PUBLIC_BASE_URL` не задан, используется `FRONTEND_URL` (старое поведение).

### Gmail: «Промоакции» vs «Основные»

Классификация решает **алгоритм Gmail** (контент, оформление, репутация домена). Технически мы помогаем так:

- **Inline-шаблоны** из репозитория (`email_templates/*.html`) — короткий preheader, белый фон, без большого «маркетингового» hero; к письму добавляется явный **text/plain** и заголовок **`List-Unsubscribe`** (если ссылка отписки HTTPS).
- **`RESEND_REPLY_TO`** — ответы на реальный ящик поддержки.
- Если письмо уходит через **опубликованный шаблон Resend**, приведите макет в Dashboard к такому же **простому** виду (мало картинок, без «акций» в теме и preheader); `text/plain` для template API Resend в том же запросе не передать — только правка шаблона в редакторе.
